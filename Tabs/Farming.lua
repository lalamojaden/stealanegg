...
local function CreateDropdownOption(Name, Order)
    ...
    Option.MouseButton1Click:Connect(function()
        ...
        if _G.Jaden_EggCheckPremium then
            local List = {}
            if SelectedRarities.Secret then table.insert(List, "Secret") end
            if SelectedRarities.Eternal then table.insert(List, "Eternal") end
            if SelectedRarities.Divine then table.insert(List, "Divine") end
            _G.Jaden_EggCheckPremium.SetRarities(List)
        end

        -- ✅ Update FarmingManager
        if _G.Jaden_FarmingManager then
            local List = {}
            if SelectedRarities.Secret then table.insert(List, "Secret") end
            if SelectedRarities.Eternal then table.insert(List, "Eternal") end
            if SelectedRarities.Divine then table.insert(List, "Divine") end
            _G.Jaden_FarmingManager.SetRarities(List)
        end
...
local function ToggleFarm()
    if not _G.Jaden_FarmingManager then
        warn("[Jaden] FarmingManager not loaded!")
        return
    end
    ...
    if FarmEnabled then
        ...
        _G.Jaden_FarmingManager.Enable()
    else
        FarmButton.BackgroundColor3 = Color3.fromRGB(28, 29, 39)
        FarmStroke.Color = Color3.fromRGB(200, 200, 220)
        _G.Jaden_FarmingManager.Disable()
    end
end
...
task.spawn(function()
    task.wait(1)
    if _G.Jaden_FarmingManager then
        local State = _G.Jaden_FarmingManager.IsEnabled()
        FarmEnabled = State
...
task.spawn(function()
    while task.wait(1) do
        if _G.Jaden_FarmingManager then
            local CurrentState = _G.Jaden_FarmingManager.IsEnabled()
            ...
            if CurrentState ~= UIState then
                ...
                print("[Jaden] Farming UI Sync | State: " .. tostring(CurrentState))
            end
        end
    end
end)
...
-- ==================================================
_G.Jaden_RefreshFarmingUI = function()
    if _G.Jaden_FarmingManager then
        local State = _G.Jaden_FarmingManager.IsEnabled()
        FarmEnabled = State
        FarmCheck.Visible = State
        ...
        print("[Jaden] Farming Tab UI Refreshed | State: " .. tostring(State))
    end
end
