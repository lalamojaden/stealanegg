...
ManagerButton.MouseButton1Click:Connect(function()
    if not _G.Jaden_ManagerDrone then
        warn("[Jaden] ManagerDrone not loaded!")
        return
    end

    local NewState = not _G.Jaden_ManagerDrone.IsEnabled()
    UpdateManagerUI(NewState)

    -- ✅ Call Enable/Disable (មិន Save Config)
    if NewState then
        _G.Jaden_ManagerDrone.Enable()
    else
        _G.Jaden_ManagerDrone.Disable()
    end
end)
...
task.spawn(function()
    task.wait(1)
    if _G.Jaden_ManagerDrone then
        local State = _G.Jaden_ManagerDrone.IsEnabled()
...
-- ✅ REFRESH FUNCTION
--==================================================
_G.Jaden_RefreshEventUI = function()
    if _G.Jaden_ManagerDrone then
        local State = _G.Jaden_ManagerDrone.IsEnabled()
        UpdateManagerUI(State)
        print("[Jaden] Event Tab UI Refreshed | State: " .. tostring(State))
    end
end
...
task.spawn(function()
    while task.wait(1) do
        if _G.Jaden_ManagerDrone then
            local CurrentState = _G.Jaden_ManagerDrone.IsEnabled()
            local UIState = ManagerCheck.Visible

            if CurrentState ~= UIState then
                UpdateManagerUI(CurrentState)
                print("[Jaden] Event UI Sync | State: " .. tostring(CurrentState))
            end
        end
    end
end)
