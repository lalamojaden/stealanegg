...
local function UpdateGetEggBox(Icon, Name, Rate, EggId)
    ...
    GetEggRate.Text = "$" .. (_G.Jaden_AutoFarm and _G.Jaden_AutoFarm.FormatMoney(Rate or 0) or tostring(Rate or 0)) .. "/s"
    SelectedEggId = EggId
...
local function ToggleGetEgg()
    ...
    if GetEggEnabled then
        ...
        if _G.Jaden_AutoFarm then
            _G.Jaden_AutoFarm.StartTeleport()
        end
    else
        GetEggCheckButton.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        GetEggCheckButton.BackgroundTransparency = 0.85
        ...
        if _G.Jaden_AutoFarm then
            _G.Jaden_AutoFarm.StopTeleport()
        end
    end
end
...
local function CreateEggEntry(EggData)
    ...
    RateLabel.Text = "$" .. (_G.Jaden_AutoFarm and _G.Jaden_AutoFarm.FormatMoney(EggData.EarningRate) or tostring(EggData.EarningRate)) .. "/s"
    ...
    SelectButton.MouseButton1Click:Connect(function()
        ...
        if _G.Jaden_AutoFarm then
            _G.Jaden_AutoFarm.SelectEgg(EggData)
        end
    end)
...
local function RefreshEggList()
    ...
    if not _G.Jaden_AutoFarm then return end
    ...
    local Eggs = _G.Jaden_AutoFarm.ScanEggs()
