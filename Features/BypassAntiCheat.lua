-- ==================================================
-- Cruuzz HUB | FEATURE | Bypass Anti Cheat
-- ==================================================
...
local function RunBypassAntiCheat()
    ...
    if not OldHumanoid then
        warn("[Cruuzz] Humanoid not found")
        return
    end

    print("========================================")
    print("[Cruuzz] START HUMANOID REPLACE")
    print("========================================")
    ...
    if not NewHumanoid then
        warn("[Cruuzz] Failed to clone Humanoid")
        return
    end
    ...
    if not NewHumanoid.Parent then
        warn("[Cruuzz] New Humanoid was removed")
        return
    end

    print("[Cruuzz] New Humanoid:", NewHumanoid)

    --==================================================
    -- UPDATE CHARACTER SYSTEM
    --==================================================
    if _G.Cruuzz_CharacterSystem then
        _G.Cruuzz_CharacterSystem.CurrentHumanoid = NewHumanoid
        _G.Cruuzz_CharacterSystem.CurrentRoot = Character:FindFirstChild("HumanoidRootPart")
        print("[Cruuzz] CharacterSystem Updated with New Humanoid")
    end
    ...
    print("[Cruuzz] Animator:", Animator)
    ...
    if Animate then
        print("[Cruuzz] Restarting Animate...")
        pcall(function()
        ...
        print("[Cruuzz] Animate restarted")
    end
    ...
    local function RefreshControls()
        ...
        if not PlayerScripts then
            warn("[Cruuzz] PlayerScripts not found")
            return
        end
        local PlayerModule = PlayerScripts:FindFirstChild("PlayerModule")
        if not PlayerModule then
            warn("[Cruuzz] PlayerModule not found")
            return
        end
        ...
        if not Success or not Module then
            warn("[Cruuzz] Failed to require PlayerModule")
            return
        end
        local Controls
        pcall(function()
        ...
        if not Controls then
            warn("[Cruuzz] Controls not found")
            return
        end
        pcall(function()
        ...
        task.wait()
        print("[Cruuzz] Controls Humanoid:", Controls.humanoid)
        print("[Cruuzz] Same Humanoid:", Controls.humanoid == NewHumanoid)
    end
    ...
    --==================================================
    -- ✅ RESTART FEATURES CHARACTER SYSTEM
    --==================================================
    if _G.Cruuzz_CharacterSystem then
        task.spawn(function()
            task.wait(0.5)
            _G.Cruuzz_CharacterSystem:RestartAllFeatures()
        end)
    end

    print("")
    print("========================================")
    print("[Cruuzz] HUMANOID REPLACE + ANTI DEATH COMPLETE")
    print("========================================")
end
