-- ==================================================
-- Jaden HUB | FEATURE | Auto Farm (Integrated & Cleaned)
-- ==================================================
...
local function EnableAutoFarm()
    ...
    print("[Jaden] Auto Farm: ON")
end

local function DisableAutoFarm()
    AutoFarmEnabled = false
    print("[Jaden] Auto Farm: OFF")
end
...
local function SelectEgg(EggData)
    SelectedEgg = EggData
    print("[Jaden] Selected Egg: " .. EggData.DisplayName .. " ($" .. FormatMoney(EggData.EarningRate) .. "/s)")
end
...
local function StartTeleport()
    if not SelectedEgg then
        warn("[Jaden] No Egg Selected")
        return
    end

    local Method = _G.Jaden_SelectedMethod or "TeleportFly"
    local Speed = _G.Jaden_TeleportSpeed or 300

    print("[Jaden] Start Teleport | Method: " .. Method .. " | Speed: " .. tostring(Speed) .. " | Target: " .. SelectedEgg.Id)

    if _G.Jaden_TeleportSystem then
        _G.Jaden_TeleportSystem.SetMethod(Method)
        _G.Jaden_TeleportSystem.SetSpeed(Speed)
        _G.Jaden_TeleportSystem.SetTargetId(SelectedEgg.Id)
        _G.Jaden_TeleportSystem.Enable()
    end
end
...
print("✅ AutoFarm Feature Loaded & Synced with Jaden Hub")
