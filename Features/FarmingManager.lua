-- ==================================================
-- JADEN HUB | FEATURE | Farming Manager (UPDATED & FIXED)
-- ==================================================
...
-- ==================================================
-- EGG CHECK PREMIUM (Integrated in FarmingManager)
-- ==================================================
...
local function StopAll()
    if _G.Jaden_AFKSystem and _G.Jaden_AFKSystem.IsEnabled() then
        local TreadmillPos = _G.Jaden_AFKSystem.GetMyTreadmillPos()
        if not TreadmillPos then
            local _, Treadmill = _G.Jaden_AFKSystem.FindMyPlotAndTreadmill()
            if Treadmill then
        ...
        if TreadmillPos then
            _G.Jaden_AFKSystem.JumpOutTreadmill(TreadmillPos, function()
                _G.Jaden_AFKSystem.Disable()
                AFKStarted = false
    ...
    if _G.Jaden_VIPTP and _G.Jaden_VIPTP.IsEnabled() then
        _G.Jaden_VIPTP.Disable()
        print("[FarmingManager] ✅ VIPTP Stopped")
    end

    if _G.Jaden_TeleportSystem and _G.Jaden_TeleportSystem.IsEnabled() then
        _G.Jaden_TeleportSystem.Disable()
...
local function StartVIPTP(EggUid)
    ...
    _G.Jaden_VIPTP.SetTargetId(EggUid)
    _G.Jaden_VIPTP.Enable()
end

-- ==================================================
-- ✅ CALLBACK FROM VIPTP (When AutoStop)
-- ==================================================
local function OnVIPTPComplete()
    ...
    if BestEgg then
        ...
        -- Fly to Safe Zone first, then start VIPTP
        task.spawn(function()
        ...
        if _G.Jaden_AFKSystem and not _G.Jaden_AFKSystem.IsEnabled() then
            _G.Jaden_AFKSystem.Enable()
            AFKStarted = true
        end
    end
end
        ...
        if BestEgg then
            ...
            if ReachedSafe then
                ...
                if IsDay and PendingEggUid then
                    ...
                    -- Wait for VIPTP to finish (Callback will invoke OnVIPTPComplete)
...
local function WaitForNight()
    print("[FarmingManager] Waiting for Night...")
    ...
    while FarmingEnabled do
        ...
        if Phase == "Night" then
            print("[FarmingManager] ✅ Night Started!")
            return true
        end
...
local function DayLoop()
    ...
    while FarmingEnabled do
        ...
        if BestEgg then
            ...
            if ReachedSafe then
                print("[FarmingManager] Waiting at Safe Zone for Night...")
                task.wait(SAFE_WAIT_AFTER_REACH)

                local IsNight = WaitForNight()

                if IsNight and PendingEggUid then
                    print("[FarmingManager] ✅ Night Reached → Start VIPTP")
                    StartVIPTP(PendingEggUid)
                    PendingEggUid = nil
...
local function Disable()
    ...
    print("[Jaden] FarmingManager: OFF")
end
